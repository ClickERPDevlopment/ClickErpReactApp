...
import { PrintEmbPIMasterType } from "@/actions/PrintingEmbroidery/print-emb-pi-action";
...
export function PrintEmbPITable({
  ...
  const columns: ColumnDef<PrintEmbPIMasterType>[] = [
    {
      accessorKey: "PI_DATE",
      header: "PI Date",
      cell: ({ row }) => (
        <div className="capitalize">{moment(row.getValue("PI_DATE")).format("DD-MMM-YYYY")}</div>
      ),
    },
    {
      accessorKey: "PARTY",
    ...
    },
    {
      ...
      header: "PI No",
      cell: ({ row }) => (
        <div className="capitalize">{row.getValue("PI_NO")}</div>
      ),
    },
    {
      id: "actions",
