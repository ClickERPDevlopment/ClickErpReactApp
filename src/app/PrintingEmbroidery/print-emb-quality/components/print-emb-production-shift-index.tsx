...
import {
  Save,
} from "@/actions/PrintingEmbroidery/print-emb-production-shift-action";
import {
  Form,
  FormControl,
  FormField,
...
function PrintEmbProductionShiftIndex() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const data = printEmbProductionShift;
  ...
  const handleDelete = async (id: number) => {
    if (id) {
      ...
      if (confirmed) {
        const response = await axios.delete(api.ProductionUrl + "/production/PrintEmbProductionShift/" + id);
  ...
  const [printEmbProductionShift, setPrintEmbProductionShift] = useState<PrintEmbProductionShiftType>({
    ID: 0,
    NAME: "",
  });
  ...
  const handleInputChange = (
    ...
    setPrintEmbProductionShift((prev) => ({
      ...prev,
...
export default PrintEmbProductionShiftIndex;
