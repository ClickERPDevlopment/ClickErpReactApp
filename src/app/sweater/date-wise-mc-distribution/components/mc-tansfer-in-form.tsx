...
import {
  transferInFormSchema,
  transferInFormType,
  TransferInAddFormSchema,
...
export default function McTransferInForm({
  selectedMcGroupId,
  firstDate,
  lastDate,
}: Props) {
  ...
  const addForm = useForm<TransferInAddFormType>({
    resolver: zodResolver(TransferInAddFormSchema),
    defaultValues: {
      MC_QTY: Number(0),
    },
  });

  const form = useForm<transferInFormType>({
    resolver: zodResolver(transferInFormSchema),
    defaultValues: {
      fromDate: new Date(firstDate),
      toDate: new Date(lastDate),
      TO_MC_GROUP_ID: selectedMcGroupId,
      details: [],
    },
  });
  ...
  function handleAddSubmit(data: TransferInAddFormType) {
    append({
      FROM_MC_GROUP_ID: data.FROM_MC_GROUP_ID,
      FROM_MC_GROUP: lstMcGroup?.find((g) => g.ID === data.FROM_MC_GROUP_ID)
        ?.GROUP_NAME!,
      MC_QTY: data.MC_QTY,
    });
  ...
  function onSubmit(data: transferInFormType) {
      ...
      <Form {...addForm}>
        ...
        >
          ...
          <div className="flex justify-end">
            <AppButton type="submit" variant={"search"}>
              Add
            </AppButton>
          </div>
        </form>
                  ...
                  return (
                    <TableRow key={field.id}>
                      <TableCell className="font-medium border border-gray-400 m-0 py-1 px-1 hidden">
                        <FormField
                          control={form.control}
                          name={`details.${index}.FROM_MC_GROUP_ID`}
                          render={({ field }) => (
                            <FormItem>
                              <FormControl>
                                <Input
                      ...
                      <TableCell className="font-medium border border-gray-400 m-0 p-0 px-1">
                        <FormField
                          control={form.control}
                          name={`details.${index}.FROM_MC_GROUP`}
                          render={({ field }) => (
                            <FormItem>
                              <FormControl>
                                <Input
