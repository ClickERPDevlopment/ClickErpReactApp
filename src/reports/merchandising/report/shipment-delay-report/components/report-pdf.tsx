...
const ReportPDF = ({ data, searchParams }: { data: IShipmentDelayReport[], searchParams: { toDate: string, fromDate: string } }) => {
    const generatePDF = () => {
        ...
        const addHeader = () => {
            // Header Title
            doc.setFontSize(13);
            doc.text('International Classic Composite Ltd.', pageWidth / 2, 15, { align: 'center' });

            // Address Info
            doc.setFontSize(9);
            doc.text('568 & 584, Naojour, Kodda, Jaydevpur, Gazipur.', pageWidth / 2, 19, { align: 'center' });
            ...
            doc.text(`${searchParams.fromDate} - ${searchParams.toDate}`, pageWidth / 2, 27, { align: 'center' });
        };
        ...
        const tableData = data.map(item => [
            item.BUYER_NAME,
            item.ORDER_PLACEMENT_MONTH ? moment(item.ORDER_PLACEMENT_MONTH).format("MMM-YY") : '',
            item.PO_NO,
            item.QTY.toString(),
            item.DELIVERY_DATE ? moment(item.DELIVERY_DATE).format("DD-MMM-YY") : '',
            item.DELIVERY_DATE ? `${moment().diff(moment(item.DELIVERY_DATE), 'days')} days` : ''
        ]);
        ...
        (doc as any).autoTable({
            ...
            didDrawPage: (data: any) => {
                ...
                doc.setFontSize(6);
                doc.text(`Page No. ${currentPage}`, pageWidth - margin - 20, doc.internal.pageSize.height - margin);

                // Add current date to bottom left
                doc.setFontSize(6);
                doc.text(`Date: ${currentDate}`, margin, doc.internal.pageSize.height - margin);
    ...
    return (
        <div className='text-right'>
            <button onClick={generatePDF} className="my-2 px-2 py-2 bg-blue-500 text-white rounded">
                Generate PDF
            </button>
        </div>
    );
