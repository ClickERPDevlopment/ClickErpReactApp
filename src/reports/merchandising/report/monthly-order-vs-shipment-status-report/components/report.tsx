...
function Report({ data, searchParams }: { data: IMonthlyOrderVsShipmentStatusReport[], searchParams: { toDate: any, fromDate: any } }) {
    ...
    //set table header 
    var firstHeader = ["MONTH", "ORDER QTY", "ORDER MIN", "ORDER VALUE", "ORDER CM", "SHIP QTY", "SHIP MIN", "SHIP VALUE", "SHIP CM"];
                    ...
                    <tbody>
                        {
                            data.map(item =>
                                <tr className='text-center odd:bg-lime-100 even:bg-white'>
                                    <td className='border border-gray-300 p-1'>{item.ORDER_DATE ? moment(item.ORDER_DATE).format("MMM-YY") : moment(item.SHIP_DATE).format("MMM-YY")}</td>
