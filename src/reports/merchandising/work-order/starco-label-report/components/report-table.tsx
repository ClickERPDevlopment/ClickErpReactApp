...
function ReportTable({data, firstHeader, sizeHeader, secondHeader}:{data:iaccWorkOrder[], firstHeader:string[]|null, sizeHeader:string[]|null, secondHeader:string[]|null}) {
  ...
  if(data)
  {
    groupedData = groupBy(data, ['STYLENO','PO_NO','PRODUCTFAMILY','MTL_NAME','MTL_COLOR_NAME','UOM','SUPPLIER_RATE_PER_PCS','DESCRIPTION','MTL_DESCRIPTION_2']);
  }
    ...
    <div className='text-sm mt-3'>
      ...
      <table className='border-collapse border border-gray-300  w-[100%]'> 
        <thead>
          <tr>
            {
                firstHeader?.map(item=><th rowSpan={2} key={item} className='border border-gray-300 p-1'>{item}</th>)
            }

            {
                (sizeHeader&&sizeHeader.length>0) ?<th colSpan={sizeHeader?.length}>SIZE RANGE WITH QTY</th>:""
            }

            {
                secondHeader?.map(item=><th rowSpan={2} key={item} className='border border-gray-300 p-1'>{item}</th>)
            }

          </tr>
          <tr>
            {
                sizeHeader?.map(item=><th key={item} className='border border-gray-300 p-1'>{item}</th>)
            }
          </tr>
        </thead>
        <tbody>
            ...
            uniqueKeysArray?.map((key, index) => {
              ...
              return (
                <tr key={key}>
                  ...
                  <td className='border border-gray-300 p-1 text-center'>{current.TOTAL_QTY}</td>
                  <td className='border border-gray-300 p-1'>{current.UOM}</td>
                  <td className='border border-gray-300 p-1'>{current.RATE}</td>
                  <td className='border border-gray-300 p-1'>{Number(current.TOTAL_AMOUNT).toFixed(2)}</td>
                  <td className='border border-gray-300 p-1'>{current.ITEM_DESCRIPTION}</td>
                  <td className='border border-gray-300 p-1'>{current.QUALITY}</td>
                </tr>
              );
            })
          }
