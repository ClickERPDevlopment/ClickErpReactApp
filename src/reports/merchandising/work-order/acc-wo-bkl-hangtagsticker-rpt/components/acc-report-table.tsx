...
export default function AccReportTable({ data }: { data: iaccWorkOrder[] }) {
    ...
    var gmtSizes = getAccReportGmtSizes(data);
    ...
    return (
        <table className="border-collapse table-fixed my-4 min-w-[100%]">
            <thead>
                <tr className='border'>
                    <th colSpan={3} className='border'>{data[0]?.MTL_NAME}</th>
                    <th colSpan={gmtSizes?.length} className='border'>Size wise Required Qty</th>
                </tr>
                <tr className='border'>
                    ...
                    {gmtSizes?.map(s => <th key={Math.random()} className='border min-w-[70px] text-sm'>{s}</th>)}
