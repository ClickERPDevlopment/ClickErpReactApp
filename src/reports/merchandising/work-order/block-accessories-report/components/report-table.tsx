...
function ReportTable({ data, firstHeader, sizeHeader, secondHeader }: { data: iaccWorkOrder[], firstHeader: string[] | null, sizeHeader: string[] | null, secondHeader: string[] | null }) {
  ...
  if (data) {
    groupedData = groupBy(data, ['STYLENO', 'PO_NO', 'GMT_COLOR_NAME', 'MTL_COLOR_NAME', 'REF_SWATCH', 'MTL_SIZE_NAME', 'MTL_NAME', 'UOM', 'DESCRIPTION', 'SUPPLIER_RATE_PER_PCS']);
  }
        ...
        <tbody>
          {
            uniqueKeysArray?.map(key =>
              <tr key={key}>
                <td className='border text-center border-gray-300 p-1'>{groupedData[key].STYLE}</td>
                <td className='border border-gray-300 p-1'>{groupedData[key].COLOR}</td>
                <td className='border border-gray-300 p-1'>{groupedData[key].MTL_COLOR}</td>
                <td className='border border-gray-300 p-1'>{groupedData[key].REF_SWATCH}</td>
                <td className='border border-gray-300 p-1'>{groupedData[key].MTL_SIZE}</td>
                ...
                <td className='border border-gray-300 p-1 text-center'>{groupedData[key].RATE.toFixed(4)}</td>
                <td className='border border-gray-300 p-1 text-center'>{Number(groupedData[key].AMOUNT).toFixed(2)}</td>
              </tr>)
          }
        </tbody>
        <tr>
          <td className='border text-center border-gray-300 p-1 font-bold' colSpan={firstHeader?.length}>Grand Total</td>
          {sizeHeader?.map(size => {
            return <td className='border border-gray-300 p-1 text-center'></td>;
          }
          )}

          <td className='border border-gray-300 p-1 text-center'>{grandTotal.TOTAL_QTY.toFixed(2)}</td>
          <td className='border border-gray-300 p-1 text-center'></td>
          <td className='border border-gray-300 p-1 text-center'>{grandTotal.AMOUNT.toFixed(2)}</td>
