...
function ReportTable({ data, firstHeader, sizeHeader, secondHeader }: { data: iaccWorkOrder[], firstHeader: string[] | null, sizeHeader: string[] | null, secondHeader: string[] | null }) {
  ...
  const totalQty = data.reduce((acc, item) => { return acc += Number(item.WORK_ORDER_QTY) }, 0)
  const totalAmount = data.reduce((acc, item) => { return acc += Number(item?.TOTAL_SUP_AMOUNT) }, 0)
  ...
  if (data) {
    groupedData = groupBy(data, ['STYLENO', 'PO_NO', 'GMT_COLOR_NAME', 'MTL_NAME', 'UOM', 'DESCRIPTION', 'SUPPLIER_RATE_PER_PCS']);
  }
    ...
    <div className='text-sm mt-3'>
      ...
      <table className='border-collapse border border-gray-300  w-[100%]'>
        ...
        <tbody>
          ...
          <tr>
            <td colSpan={6 + Number(sizeHeader?.length)} className='border border-gray-300 p-1 text-center font-bold'>Total</td>
