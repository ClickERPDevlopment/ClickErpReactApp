        ...
        <>
            {
                data.map((item, index) =>
                    <tr className='text-center'>
                        {
                            index == 0 && <td className='border border-gray-300 p-1 text-nowrap' rowSpan={data.length}>{moment(item.ENTRY_DATE).format("DD-MMM-YYYY")}</td>
                        }
                        ...
                        <td className='border border-gray-300 p-1 text-center'>{Number(item.OPERATOR) + Number(item.HP) + Number(item.IRON_MAN)}</td>
