...
function Report({ data, searchParams }: { data: IDateWiseFabricRequisitionReceive[], searchParams: { toDate: any, fromDate: any } }) {
    ...
    //set table header 
    var firstHeader = ["BUYER", "ORDER/JOB NO", "STYLE", "COLUR", "FABRIC NAME", "FABRIC SOURCE", "UNIT", "TOTAL REQUIRED", "READY QTY", "DAY REQUISITION", "DAY RECEIVE", "TOTAL RECEIVE", "BALANCE"];
                    ...
                    <tbody>
                        {
                            uniqueKeysArray?.map((key) => (
                                <ReportTable key={key} data={groupedByBuyer[key].items} firstHeader={firstHeader}></ReportTable>
                            ))
                        }
                        <tr className='text-center  bg-yellow-200'>
                            <td className='border border-gray-300 p-1 font-bold' colSpan={7}>Grand Total</td>
                            <td className='border border-gray-300 p-1'>{totalRequired.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalRedy.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalDayRequisition.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalDayReceive.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalReceive.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalBalance.toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>
                <div className='mt-5'>
                    ...
                    <table className='border-collapse border border-gray-300 mt-2'>
                        <thead>
                            <tr>
                                <th colSpan={5} className='border border-gray-300 p-1 font-bold uppercase'>Buyer Wise Summary</th>
                            </tr>
                            <tr className='bg-lime-200 text-center'>
                                {
                                    summaryHeader?.map(item => <th className='border border-gray-300 p-1'>{item}</th>)
                                }
                            </tr>
                        </thead>
                        <tbody>
                            {
                                uniqueKeysArray?.map((key) => (
                                    <ReportSummary key={key} data={groupedByBuyer[key].items} firstHeader={firstHeader}></ReportSummary>
                                ))
                            }
                            <tr className='text-center  bg-yellow-200'>
