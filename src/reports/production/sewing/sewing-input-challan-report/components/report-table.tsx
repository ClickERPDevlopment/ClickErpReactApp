    ...
    <>
      {uniqueKeysArray?.map((key) => (
        <tr style={{ fontSize: "11px" }} key={key}>
          <td className="border border-gray-950 p-0.1 text-center">{groupedByDate[key]?.PONO}</td>
          <td className="border border-gray-950 p-0.1 text-center">{groupedByDate[key]?.COLORNAME}</td>
          {sizeHeader?.map((size) =>
            <td key={size} className="border border-gray-950 p-0.1 text-center">{groupedByDate[key]?.SIZES[size] ? groupedByDate[key]?.SIZES[size] : ''}</td>
          )}
          <td className="border border-gray-950 p-0.1 text-center">{groupedByDate[key]?.QUANTITY}</td>
        </tr>
      ))}
    </>
  );
