    ...
    <>
      {
        data.map((item, index) => <tr key={index} className='text-center'>
          ...
          <td className='border border-gray-300 p-1 text-center'>{item.ACTUALHOURS}</td>
          <td className='border border-gray-300 p-1 text-center'>{(item.SEWINGOUTPUT / item.ACTUALHOURS).toFixed(2)}</td>


          <td className='border border-gray-300 p-1 text-center'>{((item.EARNINGMIN * 100) / item.AVAILABLEMIN).toFixed(2)}</td>

        </tr>)
      }
      {
        <tr className='text-center  bg-indigo-200'>
          <td className='border border-gray-300 p-1 font-bold' colSpan={3}>Total</td>
