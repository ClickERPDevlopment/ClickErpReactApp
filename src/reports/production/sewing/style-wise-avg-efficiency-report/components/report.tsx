...
function Report({ data, searchParams }: { data: IStyleWiseAvgEfficiencyReport[], searchParams: { toDate: any, fromDate: any } }) {
    ...
    //set table header 
    var firstHeader = ["BUYER", "PO", "STYLE NO", "RUN DAYS", "SMV", "PROD", "RUN MC", "USED M/O", "ACTU W/HR", "QTY PER HOUR", "OVERALL AVG EFF%"];
    ...
    return (
        <div className='px-10'>
            <div className='p-2'>
                <ReportHeader data={data[0]} searchParams={{ toDate: searchParams?.toDate, fromDate: searchParams?.fromDate }} />
                    ...
                    <tbody>
                        {
                            uniqueKeysArray?.map((key) => (
                                <ReportSubgroup key={key} data={groupedByBuyer[key].items} searchParams={searchParams}></ReportSubgroup>
                            ))
                        }
                        <tr className='text-center  bg-yellow-200'>
                            ...
                            <td className='border border-gray-300 p-1'>{(totalRunMC / data.length).toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{(totalOpeartor / data.length).toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalActualHour.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{totalQtyPerHour.toFixed(2)}</td>
                            <td className='border border-gray-300 p-1'>{((totalEarningMin * 100) / totalAvailableMin).toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>
