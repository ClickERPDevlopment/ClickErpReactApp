...
function ReportTable({ data, firstHeader }: { data: IMonthlyYarnCosting[], firstHeader: string[] | null }) {
    ...
    <>
      {
        data.map((item, index) => <tr key={index} className='text-center'>
          ...
          <td className='border border-gray-300 p-1'>{(item.QTY * item.UNIT_PRICE).toFixed(2)}</td>
        </tr>)
      }
      {
        <tr className='text-center  bg-indigo-200'>
          <td className='border border-gray-300 p-1 font-bold' colSpan={3}>Total</td>
          <td className='border border-gray-300 p-1 font-bold'>{totalQty.toFixed(2)}</td>
          <td className='border border-gray-300 p-1 font-bold'></td>
          <td className='border border-gray-300 p-1 font-bold'>{totalAmount.toFixed(2)}</td>
        </tr>
      }
    </>
  )
