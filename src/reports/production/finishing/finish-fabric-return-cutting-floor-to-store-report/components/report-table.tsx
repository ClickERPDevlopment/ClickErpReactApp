    ...
    <>
      {
        data.map((item, index) => (
          <tr key={index} className='text-center'>
            ...
            <td className='border border-gray-300 p-1'>{item.RCV_QTY.toFixed(2)}</td>
            <td className='border border-gray-300 p-1'>{(item.RCV_QTY - item.REQ_QTY).toFixed(2)}</td>
            <td className='border border-gray-300 p-1'>{item.RETURN_QTY}</td>
            <td className='border border-gray-300 p-1'>{(item.RCV_QTY - item.RETURN_QTY - item.REQ_QTY).toFixed(0)}</td>
          </tr>
        ))
      }
