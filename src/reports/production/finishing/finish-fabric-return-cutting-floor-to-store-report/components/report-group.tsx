...
function ReportGroup({ data, firstHeader }: { data: IFinishFabricReturnCuttingFloorToStoreReport[], firstHeader: string[] | null }) {

    var uniqueKeys: Set<string> = new Set();

    function groupBy(data: IFinishFabricReturnCuttingFloorToStoreReport[], keys: string[]) {
            ...
            <table className='border-collapse border border-gray-300  w-[100%]'>
                ...
                <tbody>
                    ...
                    {uniqueKeysArray?.map((key) => (
                        <ReportTable key={key} data={groupedByBuyer[key].items} ></ReportTable>
                    ))}

                    <tr className='text-center font-bold'>
                        <td className='border border-gray-300 p-1' colSpan={3}>Grand Total</td>
                        <td className='border border-gray-300 p-1'>{totalReq}</td>
                        <td className='border border-gray-300 p-1'>{totalRCV.toFixed(2)}</td>
                        <td className='border border-gray-300 p-1'>{(totalRCV - totalReq).toFixed(2)}</td>
                        <td className='border border-gray-300 p-1'>{totalReturn}</td>
                        <td className='border border-gray-300 p-1'>{(totalRCV - totalReturn - totalReq).toFixed(2)}</td>
                    </tr>
                </tbody>
            </table>
        </div>
