...
const ReportTable: React.FC<IReportTableProps> = ({ data, firstHeader }) => {
  // Calculate totals
  const calculateTotals = (): ITotals => {
    return data.reduce((acc, item) => ({
      ...
      shortExcessQty: acc.shortExcessQty + (item.ORDER_QTY - item.SHIPMENT_QTY),
  ...
  const renderTableRow = (item: IShortShipmentReasonStatus, index: number) => (
    <tr key={index} className="text-center">
      ...
      <td className="border border-gray-300 p-1">{item.ORDER_QTY - item.SHIPMENT_QTY}</td>
      <td className="border border-gray-300 p-1 text-center">
        {moment(item.SHIP_DATE).format('DD-MMM-YY')}
      </td>
      <td className="border border-gray-300 p-1 text-center">{item.REASON}</td>
      <td className="border border-gray-300 p-1 text-center">{item.AFFECTED_QTY}</td>
  ...
  const renderTotalRow = (totals: ITotals) => (
    <tr className="text-center bg-indigo-200">
      <td className="border border-gray-300 p-1 font-bold" colSpan={2}>Total</td>
