...
export default function ReportTable({
    ...
    const totalRoll = data.reduce((acc, item) => { return acc += item.ROLL_QTY }, 0)
    const totalQty = data.reduce((acc, item) => { return acc += item.QTY_KG }, 0)
                ...
                <tbody id="table-body">
                    {
                        data.map(item =>
                            <tr>
                                <td className="border text-center text-xs">{moment(item.CHALLAN_DATE).format("DD-MMM-YY")}</td>
                                ...
                                <td className="border text-center text-xs">{item.YARN_LOT_NUMBER}</td>
                                <td className="border text-center text-xs">{item.YARN_CHALLAN_REQ_DIA} & {item.GAUGE}</td>
                                ...
                                <td className="border text-center text-xs">{item.STITCH_LENGTH}</td>
                                <td className="border text-center text-xs">{item.ROLL_QTY
                                }</td>
                                <td className="border text-center text-xs">{item.QTY_KG
                                }</td>
                    ...
                    <tr>
                        <td className="border text-center text-xs font-bold" colSpan={11}>Total</td>
                        <td className="border text-center text-xs font-bold">{totalRoll}</td>
