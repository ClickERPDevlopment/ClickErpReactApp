...
import { FinishFabricAllocationSummaryReportDetailsType, FinishFabricAllocationSummaryReportMasterType } from "src/reports/finish-fabric-allocation-summary-report/finish-fabric-allocation-summary-report-type";
    ...
    return (
        <>
            <tr>
                ...
                <td className="border text-center text-xs">{totalAllocationQty?.toFixed(2)}</td>
                <td className="border text-center text-xs">{isNaN(totalAllocationQty - masterData.RECEIVE_QTY) ? "0" : (totalAllocationQty - masterData.RECEIVE_QTY).toFixed(2)}</td>
                <td className="border text-center text-xs">{masterData?.UOM}</td>
                <td className="border text-center text-xs">{Number(masterData.CONSUMPTION_PER_DZN) == 0 ? "0" : (Math.abs(totalAllocationQty - masterData.RECEIVE_QTY) / (Number(masterData.CONSUMPTION_PER_DZN) / 12)).toFixed(2)}</td>
            </tr>
        </>
    );
}
